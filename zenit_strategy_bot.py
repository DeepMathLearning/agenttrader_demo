...
class BotZenitTrendMaster(Main):
    ...
    def trend_styudy(self, req_id):
        ...
        if ((data['EMA_11'][-1] > data['EMA_55'][-1]) and 
            ...
            (data['Squeez_Momentum_Phase'][-1] == 'Impulso Alcista') and 
            (data['ADX'][-1] > 23)):
    ...
    def position_gestion(self):
        ...
        try:
            ...
            # self.positions1[self.account][self.symbol]["position"] = 0
            print(f'**** POSITION {self.positions1[self.account][self.symbol]["position"]}')
        # Aumentamos la cantidad de contratos adquiridos
        self.cant_cont = self.positions1[self.account][self.symbol]["position"]
    ...
    def estrategy_jemir(self):
        ...
        if self.accept_trade == 'a':
            ...
            self.fdata['Long_EntryA'] = np.where(
                (self.fdata[f'{self.symbol}_ADX'] > 23) &
    ...
    def strategy_metrics_jemir(self, price, market_price, sell_market_price, req_id):
        ...
        if self.accept_trade == 'ab':
            ...
            if (self.open_position and 
                ...
                #(self.fdata['EMA_34'][last_row.index[0]] < self.fdata['EMA_55'][last_row.index[0]])
                #(self.current_price < ((1 - self.stop_loss_percent) * self.average_purchase_price))
                (self.current_price < (self.average_purchase_price - (2*self.fdata['ATR'][last_row.index[0]])))
                ):
                
                self.sell(self.current_price, self.cont_1, "LIMIT")
                self.current_contracts = self.cont_1
                self.update_positions()
                ...
                if self.cant_cont == self.cant_cont_init:
                    self.operations += 1
                    self.cal_cont(1)
                    logger.info(f'se vendieron {self.current_contracts} quedan {self.cant_cont}')
                    ...
                    self.trades_info['contracts'].append(self.current_contracts)
    ...
    def close_position_method(self, last_row, req_id):
        ...
        self.sell(self.current_price, self.cont_1, "LIMIT")
        self.current_contracts = self.cont_1
        self.cont_ven += self.current_contracts
        self.update_positions()
    ...
    def update_strategies_values(self):
        ...
        self.take_profit_percent = round((self.volatility) , 4)
            ...
            <body>
                ...
                <div>
                    <center>
                        ...
                        <h2>Stop Loss price limit : {round(2*self.fdata['ATR'].iloc[-1], 3)}<h2>
                        <h2>Posición actual : {self.cant_cont} contratos<h2>
                        <h2>Último precio promedio : {self.round_to_tick(self.average_purchase_price)}<h2>
                        <h3>Posible precio de cierre : {self.round_to_tick(self.average_purchase_price * self.volatility)}<h3>
