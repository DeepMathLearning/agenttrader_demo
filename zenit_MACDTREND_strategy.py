...
class BotZenitCRUCEEMAS(Main):
    ...
    def request_firts_position(self):
        ...
        logger.info(f'**** POSITION {self.positions1[self.symbol]["position"]}')
    ...
    def get_study_data(self, req_id, interval):
        ...
        if interval == '1m':
            self.new_barsize = "1 min"  # Tamaño de las barras
        elif interval == '5m':
            self.new_barsize = "5 mins"  # Tamaño de las barras
        elif interval == '15m':
            self.new_barsize = "15 mins"  # Tamaño de las barras
        elif interval == '10m':
            self.new_barsize = "10 mins"  # Tamaño de las barras
        elif self.interval == '1h':
            self.new_barsize = "1 hour"  # Tamaño de las barras
        elif interval == '4h':
            self.new_barsize = "4 hours"  # Tamaño de las barras
        elif interval == '30m':
            self.new_barsize = "30 mins"  # Tamaño de las barras
        elif interval == '90m':
            self.new_barsize = "90 mins"  # Tamaño de las barras
        
        
        if interval == '1m':
            duration_str = "3 D"  # Duración de los datos históricos
        elif interval in ['2m', '5m','10m', '15m', '30m', '1h']:
            duration_str = "7 D"  # Duración de los datos históricos  # Duración de los datos históricos
    
        elif interval in ['4h']:
            duration_str = "10 D"  # Duración de los datos históricos

        historical_market_data = self.get_historical_market_data(self.contract, '7 D', '1 hour')
        # logger.info(self.historical_market_data[req_id])
        bar_data_dict_list = [
                                {"Date": data.date, "Open": data.open, "High": data.high, "Low": data.low, "Close": data.close, "Volume": data.volume}
    ...
    def execute_trailing_stop(self, last_row, req_id):
        if  (self.accept_trade == 'long'): 
            ...
            #     ((self.fdata['+DI'][last_row.index[0]] <= 24)) #and 
            #     #(self.adx_slope < 0)
            #     ):
    ...
    def estrategy_jemir(self):
        ...
        # Calcular indicadores necesarios
        self.fdata['EMA_5'] = ta.trend.ema_indicator(self.fdata['Close'], window=9)
        self.fdata['EMA_55'] = ta.trend.ema_indicator(self.fdata['Close'], window=21)
        
        # Calcular el MACD y la señal
        self.fdata['MACD'] = self.fdata['EMA_5'] - self.fdata['EMA_55']
        self.fdata['Signal'] = self.fdata['MACD'].ewm(span=34, adjust=False).mean()

        
        # Creamos una nueva columna
        # data1['Entry'] = 0.0
        self.fdata['Long_Signal'] = np.where((self.fdata['EMA_5'] > self.fdata['EMA_55']) &
                                              (self.fdata['MACD'] > self.fdata['Signal'])
                                              , 1.0, 0.0)
        self.fdata['Short_Signal'] = np.where((self.fdata['EMA_55'] > self.fdata['EMA_5']) &
                                               (self.fdata['MACD'] < self.fdata['Signal'])
                                               , 1.0, 0.0)
    ...
    def strategy_metrics_jemir(self, price, req_id):
        ...
        if self.accept_trade == "short":
            ...
            # Toma de ganancias
            if self.open_position and (self.fdata['MACD'][last_row.index[0]] >= self.fdata['Signal'][last_row.index[0]]):  
    ...
    def html_generate(self):
        ...
        if self.with_trend_study:
            file_name = f"bot_activity/{self.strategy_name}_smart_{self.interval}_{self.symbol}_{self.hora_ejecucion}.html"
        else:
            file_name = f"bot_activity/{self.strategy_name}_{self.accept_trade}_{self.interval}_{self.symbol}_{self.hora_ejecucion}.html"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--symbol', type=str, default='MESM4', help='symbol example AAPL')
    ...
    parser.add_argument('--account', type=str, default='DU7186454', help='Account')
    ...
    parser.add_argument('--lastTradeDateOrContractMonth', type=str, default="20240621", help='The expire date for futures')
