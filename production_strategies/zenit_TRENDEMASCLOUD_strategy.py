...
class BotZenitTRENDEMASCLOUD(Main):
    ...
    def trend_styudy(self, req_id, interval):
        ...
        if ((data['EMA_11'][-1] > data['EMA_55'][-1])): # and (data['MACD'][-1] > data['Signal'][-1])):
            self.accept_trade = 'long'
        elif ((data['EMA_11'][-1] < data['EMA_55'][-1])): # and (data['MACD'][-1] < data['Signal'][-1])):
    ...
    def estrategia_trading(self):
        ...
        self.fdata['EMA55'] = self.calcular_ema(self.fdata, 50)
        ...
        self.fdata['Long_Signal'] = np.where(
                                            ...
                                            (self.fdata['EMA9'] > self.fdata['EMA12']) &
                                            (self.fdata['EMA12'] > self.fdata['EMA21']) & 
                                            (self.fdata['EMA21'] > self.fdata['EMA34']) &
                                            (self.fdata['EMA34'] > self.fdata['EMA55']) &
                                            (self.fdata['ADX'] >= self.di_plus) &
                                            (self.fdata['+DI'] >= self.di_plus) &
                                            (self.fdata['+DI'] >= self.fdata['-DI']),1,0)
    ...
    def strategy_metrics_(self, price, req_id):
        ...
        self.min_short= ((self.fdata['EMA9'][last_row.index[0]] < self.fdata['EMA21'][last_row.index[0]]) and
                        (self.fdata['EMA21'][last_row.index[0]] < self.fdata['EMA34'][last_row.index[0]]) and
                        (self.fdata['EMA34'][last_row.index[0]] < self.fdata['EMA55'][last_row.index[0]]))
        self.min_long = ((self.fdata['EMA9'][last_row.index[0]] > self.fdata['EMA21'][last_row.index[0]]) and
                        (self.fdata['EMA21'][last_row.index[0]] > self.fdata['EMA34'][last_row.index[0]]) and
                        (self.fdata['EMA34'][last_row.index[0]] > self.fdata['EMA55'][last_row.index[0]]))
...
############################################# #EMA 21
                ...
                (self.current_price >= self.fdata['EMA21'][last_row.index[0]]) and
                (self.current_price < self.fdata['EMA34'][last_row.index[0]]) and
                self.min_short
                ):
                print(f'****** SAFETY ORDER *1* {self.active_safety_orders}')
                self.open_trade_price1 = self.current_price
                
                self.sell(self.open_trade_price1, float(self.so_1), "LIMIT")
                ...
                if self.cant_cont <= self.cant_cont_init + ((-1)* self.cont_1):
                    ...
                    (self.fdata['ADX'][last_row.index[0]] >= self.di_minus) and
                    (self.fdata['-DI'][last_row.index[0]] >= self.di_minus) and
                    (self.fdata['EMA34'][last_row.index[0]] > self.fdata['EMA21'][last_row.index[0]]) and
                    (self.fdata['EMA55'][last_row.index[0]] > self.fdata['EMA34'][last_row.index[0]]) and
                    #(self.current_price >= self.fdata['EMA34'][last_row.index[0]]) and
                    #(self.current_price < self.fdata['EMA55'][last_row.index[0]]) and
                    (self.current_price >= self.fdata['EMA55'][last_row.index[0]]) and
                    self.min_short
                    ):   
                    ...
                    self.sell(self.open_trade_price2, float(self.so_2), "LIMIT")
                    self.update_positions()
    ...
    #########################################
                if (
                    ...
                    self.buy(self.current_price, float(self.cant_contracts), "LIMIT")
    ...
    #########################################
                      ...
                      (self.fdata['EMA21'][last_row.index[0]] >= self.fdata['EMA55'][last_row.index[0]])
                        ):  
            ...
            if ((not self.buy_ema9) and
                ...
                (self.current_price <= self.fdata['EMA9'][last_row.index[0]]) and
                (self.current_price >= self.fdata['EMA12'][last_row.index[0]])
                ): 
    ...
    #########################################
                ...
                self.buy(self.open_trade_price1, float(self.so_1), "LIMIT")
                ...
                if self.cant_cont >= self.cant_cont_init + self.cont_1:
                    ...
                    (self.fdata['+DI'][last_row.index[0]] >= self.di_plus) and
                    (self.fdata['EMA21'][last_row.index[0]] > self.fdata['EMA34'][last_row.index[0]]) and
                    (self.fdata['EMA34'][last_row.index[0]] > self.fdata['EMA55'][last_row.index[0]]) and
                    # (self.current_price <= self.fdata['EMA34'][last_row.index[0]]) and
                    # (self.current_price >= self.fdata['EMA55'][last_row.index[0]]) and
                    (self.current_price <= self.fdata['EMA55'][last_row.index[0]]) and
                    self.min_long
                    ...
                    self.buy(self.open_trade_price2, float(self.so_2), "LIMIT")
                    self.update_positions()
...
#####################################
            ...
            # Toma de ganancias  
                if ( 
                    ...
                    self.sell(self.current_price, float(self.cant_contracts), "LIMIT")
...
#####################################  
    ...
    def check_trailing_stop_activate(self,last_row):
        ...
        if ( (self.accept_trade == 'long') and
            ...
            print('TRAILING STOP ACTIVADO PRICE')

        elif ( (self.accept_trade == 'long') and
        ...
        elif ( (self.accept_trade == 'short') and
            (self.current_price <= (self.average_purchase_price * (1-self.volatility)))
            ): 
            self.trailing_stop_activate_price = True
            print(f'TRAILING STOP ACTIVADO PRICE EN {self.current_price}')
    ...
    def execute_trailing_stop(self, last_row, req_id):
        if (self.accept_trade == 'long'):
            ...
            if ((self.trailing_stop_DTC) and
                (self.current_price <= self.average_purchase_price + (self.DTC - (self.tick_value*2) )) 
                ):
...
if __name__ == '__main__':
    ...
    bot = BotZenitTRENDEMASCLOUD(args.ip, 
              args.port, 
              args.client, 
              args.symbol, 
              args.secType, 
